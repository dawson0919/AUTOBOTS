"""
BTC Signal Engine — Derives trading signals from Polymarket BTC markets
======================================================================
Fetches BTC markets from Polymarket, analyzes probabilities,
produces composite bull/bear signals with price targets.

Usage:
    python btc_signal.py              # Show signal
    python btc_signal.py --json       # JSON output for dashboard
"""
...
def fetch_btc_markets() -> list[dict]:
    """Fetch active BTC-related markets from Polymarket Gamma API."""
    url = f"{GAMMA_API}/markets?limit=100&active=true&order=volume24hr&ascending=false"
    resp = httpx.get(url, timeout=15)
    all_markets = resp.json()

    btc_keywords = re.compile(r"\b(btc|bitcoin)\b", re.IGNORECASE)
    price_keywords = re.compile(
        r"(price|above|below|up|down|range|\$[\d,]+|↑|↓)", re.IGNORECASE
    )
    exclude_keywords = re.compile(
        r"(etf|approval|regulation|sec |reserve|strategic|dominance|halving)", re.IGNORECASE
    )

    results = []
    for m in all_markets:
        q = m.get("question", "") or m.get("title", "")
        if not btc_keywords.search(q):
            continue
...
def suggest_strategies(signal: dict) -> list[dict]:
    ...
    if signal.get("topRange"):
        tr = signal["topRange"]
        strategies.append({
            "name": f"目標 ${tr['low']//1000}k-${tr['high']//1000}k",
...
def main():
    parser = argparse.ArgumentParser(description="BTC Signal Engine — Polymarket")
    parser.add_argument("--json", action="store_true", help="JSON output for dashboard")
    args = parser.parse_args()

    raw = fetch_btc_markets()
    ...
    if args.json:
        ...
        if not signal:
            print("No BTC markets found on Polymarket.")
            sys.exit(1)

        sc = signal["composite"]
        arrow = "▲" if sc > 0.15 else ("▼" if sc < -0.15 else "─")
        print(f"\n{'='*50}")
        print(f"  ₿ BTC Signal — {signal['label']}  {arrow}")
        print(f"{'='*50}")
